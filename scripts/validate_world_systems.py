...
REFERENCE_MAP={'rotom':'electivire','squirtle':'lapras','pacific-northwest':'pacific-northwest-temperate-rainforest','central-honshu':'central-honshu-urban-mountain-corridor'}
REMOVED_INCIDENT_REFERENCES={'gaia-i-2020-041'};errors=[]
...
def main():
    ...
    for incident_id in REMOVED_INCIDENT_REFERENCES:require(incident_id in correction_source,f'Systems correction does not document orphan removal: {incident_id}')
    require('knownIncidentIds' in correction_source and '.filter(id=>knownIncidentIds.has(id))' in correction_source,'Systems correction does not filter orphan incident references against the effective archive')
    ...
    raw_incident_refs=set(array_values(system_source,'incidents'));raw_incident_refs.update(value for value in re.findall(r"incidentId:(?:'([^']*)'|null)",evidence_source) if value);effective_incident_refs=raw_incident_refs-REMOVED_INCIDENT_REFERENCES;foundation_incident_ids={row['id'] for row in full_canon.get('incidents',[])};known_incidents=set(incident_ids)|foundation_incident_ids;require(not sorted(effective_incident_refs-known_incidents),f'Unknown effective investigation cross-references: {sorted(effective_incident_refs-known_incidents)}')
    require(REMOVED_INCIDENT_REFERENCES <= raw_incident_refs,'Documented orphan incident removal no longer corresponds to raw source')
    ...
    print(f'GAIA World Systems & Evidence Pass I validated: 8 systems, 9 original evidence plates, 8 new investigations, 3 lineage pilots, corrected references inside signed canon, orphan reference removed, no population mutation');return 0
