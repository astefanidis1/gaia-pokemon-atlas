...
The current branch combines **Release Candidate 1 (`2026-07-29.1`)** with **World Completion Pass I (`2026-08-01.1`)**. It remains a draft, unmerged, undeployed, and unpromoted.
...
- local Discovered, location-specific Observed, and Favorite visitor states;
- **27 full dossiers** with founder notes, public advisories, and linked archives;
- a deliberate **Civilian Summary Record** tier so every other species remains a complete public record rather than unfinished content;
- **six Regional Field Windows**: New England, the Aegean / Eastern Mediterranean, the Pacific Northwest, Central Honshu, the Central Andes, and the East African Rift Highlands;
...
- **24 ecological relationships** covering predation, pollination, infrastructure, working partnerships, wetland facilitation, geological disturbance, and other ecosystem functions;
...
- an expanded **GAIA Live** world-state surface combining migration, current regional seasons, active ecology, and causal relationships;
- canonical-location observation records stored only in the visitor’s browser, with impossible or withheld observations blocked;
- a readable in-universe archive-document viewer with previous/next navigation and shareable document links;
- Index filters for danger, mobility, publication depth, realm, category, and multiple sorting modes;
...
Readable application source lives in `src/app/*.js`. The browser fetches all thirteen modules in parallel and executes them in the locked reviewed order.
...
CI generates the release image package, materializes the exact production-shaped HTML, compiles the Python tooling, syntax-checks every readable JavaScript module, validates Markdown as strict UTF-8, verifies every canon/editorial version chain, enforces performance budgets, runs the complete browser/network/offline/completion matrix, applies axe-core accessibility checks, and uploads reports plus desktop/mobile review captures. The future Pages workflow repeats the same gates before deployment is permitted.
...
Signed population canon + versioned corrections
                         ...
                         + World Completion regional/editorial layer
                         + public navigation and continuity layer
                         + replaceable visual presentation policy
                         + guarded release-candidate presentation
```

Editorial, regional, ecology, navigation, visual, assurance, completion, and release layers deepen or protect the experience without silently changing exact census totals.
...
tests/                        Playwright experience, layout, RC, and completion assurance
